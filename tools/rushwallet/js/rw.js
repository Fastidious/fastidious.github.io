    ...
    "getHistory": function ()
    {
        ...
        }).done(function (msg)
        {
            ...
            for ( i=0;i<msg.data.txs.length;i++ )
            {
                ...
                $("#txTable tbody").append( '<tr><td>' + txTime + '</td><td class="hidden-sm hidden-xs"><a href="https://blockchain.info/tx/' + msg.data.txs[i].tx + '" target="_blank" >' + msg.data.txs[i].tx.substring(0,30) + '...</a></td><td class="hidden-sm hidden-xs">' + msg.data.txs[i].confirmations + '</td><td>' + btcFormat( msg.data.txs[i].amount ) + '</td></tr>' );
            }
