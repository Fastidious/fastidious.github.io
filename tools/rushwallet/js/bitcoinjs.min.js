    ...
    return function e(t, n, r) {
        ...
        2: [function(require, module, exports) {
            ...
            module.exports.checkDecode = function(input) {
                var bytes = module.exports.decode(input), front = bytes.slice(0, bytes.length-4), back = bytes.slice(bytes.length-4);
                var checksum = Crypto.SHA256(Crypto.SHA256(front, {
                    asBytes: true
                }), {
                    asBytes: true
        ...
        3: [function(require, module, exports) {
            var Script = require("./script"), util = require("./util"), conv = require("./convert"), key = require("./eckey"), base58 = require("./base58"), Crypto = require("./crypto-js/crypto"), ECPointFp = require("./jsbn/ec").ECPointFp, sec = require("./jsbn/sec"), ecparams = sec("secp256k1");
            ...
            BIP32key.prototype.deserialize = function(str) {
                ...
                var front = bytes.slice(0, bytes.length-4), back = bytes.slice(bytes.length-4);
                var checksum = Crypto.SHA256(Crypto.SHA256(front, {
                    asBytes: true
                }), {
                    asBytes: true
            ...
            BIP32key.prototype.ckd = function(i) {
                ...
                if (i >= 2147483648) {
                    if (this.priv)
                        throw new Error("Can't do private derivation on public key!");
                    blob = [0].concat(priv.slice(0, 32), util.numToBytes(i, 4).reverse())
                } else 
                    blob = pub.concat(util.numToBytes(i, 4).reverse());
        ...
        4: [function(require, module, exports) {
            ...
            module.exports.coerceToBytes = function(input) {
                if (typeof input == "string")
                    return module.exports.hexToBytes(input);
                return input
            };
        ...
        9: [function(require, module, exports) {
            ...
            module.exports = function(message, options) {
                var H = [[1779033703, 4089235720], [3144134277, 2227873595], [1013904242, 4271175723], [2773480762, 1595750129], [1359893119, 2917565137], [2600822924, 725511199], [528734635, 4215389547], [1541459225, 327033209]];
                if (message.constructor === String) {
                    message = conv.stringToBytes(message)
                }
                var m = util.bytesToWords(message);
        ...
        13: [function(require, module, exports) {
            var BigInteger = require("./jsbn"), sec = require("./sec");
            ...
            ECPointFp.decodeFrom = function(ecparams, enc) {
                ...
                if (type == 4) {
                    var xBa = enc.slice(1, 1 + dataLen / 2), yBa = enc.slice(1 + dataLen / 2, 1 + dataLen), x = BigInteger.fromByteArrayUnsigned(xBa), y = BigInteger.fromByteArrayUnsigned(yBa)
                } else {
                    var xBa = enc.slice(1), x = BigInteger.fromByteArrayUnsigned(xBa), p = ecparams.getQ(), xCubedPlus7 = x.multiply(x).multiply(x).add(new Bitcoin.BigInteger("7")).mod(p), pPlus1Over4 = p.add(new Bitcoin.BigInteger("1")).divide(new Bitcoin.BigInteger("4")), y = xCubedPlus7.modPow(pPlus1Over4, p);
        ...
        21: [function(require, module, exports) {
            ...
            Transaction.prototype.sign = function(index, key, type) {
                ...
                key = new ECKey(key);
                var pub = key.getPub(), hash160 = util.sha256ripe160(pub), script = Script.createOutputScript(new Address(hash160)), hash = this.hashTransactionForSignature(script, index, type), sig = key.sign(hash).concat([type]);
            ...
            Transaction.prototype.signWithKeys = function(keys, outputs, type) {
                ...
                for (var i = 0; i < this.ins.length; i++) {
                    var outpoint = this.ins[i].outpoint.hash + ":" + this.ins[i].outpoint.index, histItem = hmap[outpoint];
                    if (!histItem)
                        continue;
                    var thisInputAddrdata = addrdata.filter(function(a) {
                        return a.address == histItem.address
                    });
                    if (thisInputAddrdata.length == 0)
                        continue;
            ...
            var TransactionOut = function(data) {
                ...
                if (this.script.buffer.length > 0)
                    this.address = this.script.toAddress();
            ...
            module.exports = {
                ...
                numToVarInt: function(num) {
                    ...
                    if (num < 253)
                        ...
                        return [253].concat(m.numToBytes(num, 2));
                    else if (num < 4294967296)
                        return [254].concat(m.numToBytes(num, 4));
                    else 
                        return [253].concat(m.numToBytes(num, 8))
                },
            ...
            var Wallet = function(seed) {
                ...
                this.mkSend = function(to, value, fee) {
                    ...
                    var sum = utxo.reduce(function(t, o) {
                        return t + o.value
                    }, 0), remainder = sum - value - fee;
                    if (value < 5430)
                        throw new Error("Amount below dust threshold!");
                    var unspentOuts = 0;
                    for (var o in this.outputs) {
                        if (!this.outputs[o].spend)
                            unspentOuts += 1;
                        if (unspentOuts >= 5)
                            return 
                    }
                ...
                this.mkSendToOutputs = function(outputs, changeIndex, fee) {
                    var value = outputs.reduce(function(t, o) {
                        return t + o.value
                    }, 0), utxo = this.getUtxoToPay(value + fee), sum = utxo.reduce(function(t, p) {
                        return t + o.value
                    }, 0);
                ...
                this.sign = function(tx) {
                    tx.ins.map(function(inp, i) {
                        ...
                        if (me.outputs[inp]) {
                            var address = me.outputs[inp].address, ind = me.addresses.indexOf(address);
                            if (ind >= 0) {
                                var key = keys[ind];
                                tx.sign(ind, key)
                            }
                ...
                this.getPrivateKey = function(index) {
                    if (typeof index == "string")
                        return keys.filter(function(i, k) {
